...
def extract_player_data(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Player name - try multiple locations
    ...
    # Avatar ID: from profileInfo.avatarId (as you specified)
    avatar_id = profile.get("avatarId")
    if not avatar_id:
        avatar_id = basic.get("headPic")
    avatar_id = str(avatar_id) if avatar_id else ""
    
    # Banner ID: from profileInfo.bannerId (as you specified)
    banner_id = profile.get("bannerId")
    if not banner_id:
        banner_id = basic.get("bannerId")
    banner_id = str(banner_id) if banner_id else ""
    
    # Prime level: from primeInfo.primeLevel (as you specified)
    prime_level = prime_info.get("primeLevel", 0)
    if not prime_level:
