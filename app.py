...
# Outfit positions (based on the outfit.png template)
HEX_POSITIONS = {
    "mask": (990, 420),
    "shirt": (190, 90),
    "pants": (40, 420),
...
async def lifespan(app: FastAPI):
    ...
    if os.path.exists(OUTFIT_BACKGROUND):
        ...
        logger.warning(f"⚠️ Outfit background not found: {OUTFIT_BACKGROUND}")
...
def fetch_sync_image_bytes(item_id: str) -> Optional[bytes]:
    """Synchronous version for outfit generator."""
    if not item_id or str(item_id).lower() in ("0", "none", "null", ""):
        return None
    url = f"{CDN_URL}/{item_id}.png"
    try:
...
def fetch_icon(icon_id, size=ICON_SIZE, is_character=False):
    """Fetch outfit icons (character or normal)."""
    try:
        if is_character:
...
async def fetch_real_player_data(uid: str) -> Dict[str, Any]:
    ...
    # clothes order: [mask, shirt, pants, shoes, emote, armor]
    ...
    logger.info(f"📊 Extracted: name='{name}', level={level}, guild='{guild}', prime={prime_level}, clothes={len(clothes)} items")
...
# ================= Outfit Generation with Overrides =================
def generate_outfit_image(outfit_data: Dict[str, Any]) -> io.BytesIO:
    if not os.path.exists(OUTFIT_BACKGROUND):
        raise FileNotFoundError(f"Background template '{OUTFIT_BACKGROUND}' not found. Please upload this file.")
    
    canvas = Image.open(OUTFIT_BACKGROUND).convert("RGBA")
    
    # Extract slots with priority: override > real data
    draw_tasks = {
    ...
    for slot, item_id in draw_tasks.items():
        ...
        if slot == "character":
            ...
            if not icon_img:
                logger.warning(f"Character icon not found: {item_id}")
                continue
            w, h = icon_img.size
            ...
            if not icon_img:
                logger.warning(f"Icon not found for {slot}: {item_id}")
                continue
...
async def generate_outfit(
    uid: str = Query(...),
    # Custom overrides for each slot
    ...
    """Generate outfit image with optional custom overrides for each slot."""
    try:
        real_data = await fetch_real_player_data(uid)
        clothes = real_data.get("clothes", [])
        
        # Build outfit data: override takes precedence, otherwise real data
        outfit_data = {
        ...
        logger.info(f"👕 Generating outfit for UID: {uid} with overrides: head={head}, mask={mask}, top={top}, pants={pants}, shoes={shoes}, faceprint={faceprint}, paint={paint}, weapon={weapon}, pet={pet}")
        ...
        logger.error(f"Outfit background missing: {e}")
        raise HTTPException(status_code=500, detail=f"Missing background file: {str(e)}")
    except Exception as e:
