...
# New fonts (old fonts removed)
FONT_CHEROKEE = "NotoSansCherokee.ttf"
FONT_DEVANAGARI = "gff-devanagari-regular.ttf"
FONT_DEFAULT = "hindsiliguri.ttf"
...
def load_unicode_font(size: int, font_file: str) -> ImageFont.FreeTypeFont:
    try:
        font_path = os.path.join(os.path.dirname(__file__), font_file)
        if os.path.exists(font_path):
            return ImageFont.truetype(font_path, size)
    except Exception as e:
        logger.warning(f"Font error for {font_file}: {e}")
...
def get_font_for_char(size: int, char: str):
    """Return the appropriate font based on character Unicode block."""
    if is_cherokee(char):
        return load_unicode_font(size, FONT_CHEROKEE)
    elif is_devanagari(char):
        return load_unicode_font(size, FONT_DEVANAGARI)
    else:
        return load_unicode_font(size, FONT_DEFAULT)

def draw_text_stroked(draw, x, y, text, default_font_size, stroke=3):
    """Draw text with black stroke, picking font per character."""
    cx = x
    for ch in text:
        font = get_font_for_char(default_font_size, ch)
...
def extract_player_data(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.info(f"Extracted: name='{name}', level={level}, guild='{guild}', avatar_id={avatar_id}, banner_id={banner_id}, prime_level={prime_level}")
    return {
        "name": name,
        "level": level,
        "guild": guild,
...
def process_banner_image(avatar_bytes: Optional[bytes],
    ...
    # Prime badge overlay (top-left corner)
    ...
    font_name_size = 110
    name_lines = wrap_text(player["name"], get_font_for_char(font_name_size, "A"), max_text_width)
    y = 40
    for line in name_lines:
        draw_text_stroked(draw, name_x, y, line, font_name_size, 4)
        y += 85
    ...
    if player["guild"]:
        draw_text_stroked(draw, name_x, y, player["guild"], 80, 3)

    # Level badge (bottom right)
    lvl_text = f"Lvl.{player['level']}"
    font_level = load_unicode_font(50, FONT_DEFAULT)
    try:
