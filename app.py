...
def extract_player_data(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if prime_level is None and "primeLevel" in prime_info:
...
def process_banner_image(headPic_bytes: Optional[bytes],
    ...
    if prime_img is not None:
        ...
        logger.warning(f"Prime badge image for level {player['prime_level']} not found")
    ...
    # ========== EXTRA SPACING BEFORE GUILD (Far below player name) ==========
    y += 50   # Large gap between player name and guild name

    # Guild name (far below)
    if player["guild"]:
