...
async def fetch_image_bytes(item_id: str) -> Optional[bytes]:
    if not item_id or str(item_id).lower() in ("0", "none", "null", ""):
        logger.debug(f"Skipping invalid ID: {item_id}")
        return None
...
def bytes_to_image(img_bytes: Optional[bytes]) -> Image.Image:
    ...
    # Return a neutral grey square instead of crashing
...
def extract_player_data(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Avatar ID: PRIORITIZE profileInfo.avatarId (example: 102000054)
    avatar_id = profile.get("avatarId")
    if not avatar_id:
        avatar_id = basic.get("headPic")
    if not avatar_id:
        avatar_id = captain.get("headPic")
    avatar_id = str(avatar_id) if avatar_id else ""

    # Banner ID: PRIORITIZE basicInfo.bannerId (example: 901046014)
    banner_id = basic.get("bannerId")
    if not banner_id:
        banner_id = profile.get("bannerId")
    if not banner_id:
        banner_id = captain.get("bannerId")
...
def process_banner_image(avatar_bytes: Optional[bytes],
    ...
    # Layout: player name (wrapped) then guild name with extra spacing
    ...
    for line in name_lines:
        ...
        y += 85  # line height for name

    # Extra spacing before guild name (now significantly below)
    y += 40   # <-- increased gap between name and guild
