...
import re
import json
import asyncio  # <--- IMPORTANT: ensure this line exists
...
app = FastAPI(lifespan=lifespan, title="FF Banner API", description="Powerful banner generator with extra endpoints")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
async def root():
    return {
        "endpoints": {
            ...
            "/batch?uids={uid1},{uid2}": "Generate multiple banners (ZIP file)",
            "/prime/levels": "List all prime levels with badge info",
...
async def player_info(uid: str = Query(..., description="Player UID")):
    """Proxy to external player info API."""
    try:
        resp = await app.state.client.get(f"{INFO_API_URL}?uid={uid}")
        if resp.status_code != 200:
...
@app.get("/batch")
async def batch_banners(uids: str = Query(..., description="Comma-separated UIDs, e.g. 123,456")):
    """Generate multiple banners and return as a ZIP file."""
    import zipfile
    ...
    with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zip_file:
        for uid in uid_list:
            try:
                ...
                logger.warning(f"Failed to generate banner for {uid}: {e}")
                continue
    zip_buffer.seek(0)
...
async def prime_levels():
    """List all prime levels with badge names."""
    return {
        "prime_levels": [
