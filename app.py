...
def extract_player_data(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Avatar ID: use profileInfo.avatarId (as you requested)
    avatar_id = profile.get("avatarId")
    if not avatar_id:
        avatar_id = basic.get("headPic")  # fallback
    avatar_id = str(avatar_id) if avatar_id else ""

    # Banner ID: use basicInfo.bannerId (as you requested)
    banner_id = basic.get("bannerId")
    if not banner_id:
        banner_id = profile.get("bannerId")  # fallback
