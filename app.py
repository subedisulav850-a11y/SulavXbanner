...
# Fonts (optional – place in same folder)
...
app = FastAPI(lifespan=lifespan, title="FF Banner API", description="Powerful banner generator with player-info proxy")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
# ================= Data Extraction from External API =================
async def fetch_real_player_data(uid: str) -> Dict[str, Any]:
    """Fetch player info from external API and extract relevant fields."""
    ...
    if prime_level is None and "level" in prime_info:
        prime_level = prime_info.get("level")
...
def generate_banner_image(headPic_bytes: Optional[bytes],
    ...
    # Extra spacing before guild (far below)
    y += 50
...
async def root():
    ...
    return {
        "endpoints": {
            "/": "This help page",
            "/health": "Health check",
            "/player-info?uid={uid}": "Proxy to external player info API (raw JSON)",
        ...
        "example": "/profile?uid=3074306062",
        "docs": "/docs"
    }
...
@app.get("/player-info")
async def player_info(uid: str = Query(..., description="Player UID")):
    """Proxy the external player info API."""
    try:
        resp = await app.state.client.get(f"{INFO_API_URL}?uid={uid}")
        if resp.status_code != 200:
...
async def get_banner(
    ...
    bannerid: Optional[str] = Query(None, description="Override banner ID"),
    avatarid: Optional[str] = Query(None, description="Override avatar ID (headPic)"),
    primelevel: Optional[int] = Query(None, ge=0, le=8, description="Override prime level (0-8)"),
    guildname: Optional[str] = Query(None, description="Override guild name"),
    playername: Optional[str] = Query(None, description="Override player name"),
    level: Optional[str] = Query(None, description="Override account level")
):
    try:
        ...
        avatar_bytes = await fetch_image_bytes(final_data["headPic"])
        banner_bytes = await fetch_image_bytes(final_data["banner_id"])
...
@app.get("/random")
async def random_banner(uid: str = Query(..., description="Player UID")):
    """Generate banner with random avatar, banner, and prime level (name/guild remain real)."""
    try:
        real_data = await fetch_real_player_data(uid)
