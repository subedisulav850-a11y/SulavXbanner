...
# ================= Data Extraction (PRIME LEVEL 0 FIXED) =================
def extract_player_data(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Avatar: use headPic
    headPic = profile.get("headPic")
    if not headPic:
        headPic = basic.get("headPic")
    headPic = str(headPic) if headPic else ""
    ...
    # ========== FIXED: PRIME LEVEL EXTRACTION (ALWAYS RETURNS 0-8) ==========
    prime_level = None
    
    # Try every possible location where primeLevel might exist
    locations_to_check = [
        ("primeInfo", "primeLevel"),           # primeInfo.primeLevel (most common)
        ("profileInfo", "primeLevel"),         # profileInfo.primeLevel
        ("basicInfo", "primeLevel"),           # basicInfo.primeLevel
        (None, "primeLevel"),                  # root level primeLevel
        ("primeInfo", "level"),                # primeInfo.level (alternative)
        ("profileInfo", "level"),              # profileInfo.level (but this is account level, not prime)
    ]
    
    for obj_name, key in locations_to_check:
        if obj_name is None:
            # Check root level
            if key in data:
                prime_level = data.get(key)
                if prime_level is not None:
                    break
        else:
            obj = data.get(obj_name, {})
            if key in obj:
                prime_level = obj.get(key)
                if prime_level is not None:
                    break
    ...
    # Ensure it's a valid integer between 0 and 8
    try:
    ...
    logger.info(f"Extracted: name='{name}', level={level}, guild='{guild}', headPic={headPic}, banner_id={banner_id}, prime_level={prime_level}")
...
# ================= Image Processing =================
def process_banner_image(headPic_bytes: Optional[bytes],
                         banner_bytes: Optional[bytes],
                         player: Dict[str, Any]) -> io.BytesIO:
    TARGET_HEIGHT = 400

    # --- Avatar (using headPic) ---
    ...
    if player["prime_level"] == 8 and app.state.prime8_frame is not None:
        try:
            ...
            logger.info("Applied prime 8 frame")
        except Exception as e:
            logger.warning(f"Frame composite failed: {e}")

    # Prime badge overlay (top-left corner) - WORKS FOR ALL LEVELS 0-8
    ...
    if prime_img is not None:
        try:
            ...
            logger.info(f"Applied prime {player['prime_level']} badge")
        except Exception as e:
            logger.warning(f"Prime badge overlay error: {e}")
    else:
        logger.warning(f"Prime badge image for level {player['prime_level']} not found, using placeholder")
    ...
    # Layout: player name (wrapped) then guild name (no extra blank lines)
...
async def get_banner(uid: str):
    try:
        ...
        # Fetch images
        headPic_bytes, banner_bytes = await asyncio.gather(
