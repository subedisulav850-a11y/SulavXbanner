...
def extract_player_data(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Avatar ID: try profileInfo.avatarId, then basicInfo.headPic, then captainBasicInfo.headPic
    avatar_id = profile.get("avatarId") or basic.get("headPic") or captain.get("headPic")
    # Banner ID: try basicInfo.bannerId, then profileInfo.bannerId, then captainBasicInfo.bannerId
    banner_id = basic.get("bannerId") or profile.get("bannerId") or captain.get("bannerId")
    ...
    logger.info(f"Extracted: {name}, Lv.{level}, Guild:{guild}, Avatar:{avatar_id}, Banner:{banner_id}, Prime:{prime_level}")
    return {
...
def process_banner_image(avatar_bytes: Optional[bytes],
    ...
    # Prime badge overlay on avatar (top-left corner)
    prime_img = app.state.prime_images.get(player["prime_level"])
    if prime_img is not None:
        try:
            badge_size = 70   # slightly smaller to fit better
    ...
    try:
        ...
        logger.warning(f"Banner processing error: {e}")
        banner_img = Image.new("RGBA", (800, 400), (100, 100, 100, 255))
    ...
    # Layout: player name (wrapped) then guild name directly below
...
async def get_banner(uid: str):
    try:
        ...
        logger.exception(f"Unexpected error for UID {uid}")
