...
async def root():
    return {
        "endpoints": {
            "/": "This help",
            "/health": "Health check",
            "/player-info?uid={uid}": "Raw player data",
            "/banner?uid={uid}": "Generate banner",
            "/random-banner?uid={uid}": "Random prime level banner",
            "/batch-banners?uids=...": "ZIP of banners",
            "/outfit?uid={uid}": "Real outfit",
            "/outfit?...": "Custom outfit overrides",
            "/prime-levels": "List prime levels",
            "/eat-access?eat={token}": "EAT token → access token",
            "/access-jwt?access_token={token}": "Access token → JWT",
            "/token?uid={uid}&password={pwd}": "UID/password → JWT",
            "/token/batch": "POST JSON array for batch JWT",
            "/level?uid={uid}": "Level progress",
            "/bancheck?uid={uid}": "Ban status",
            "/region?uid={uid}": "Region info"
        },
        "docs": "/docs"
    }
...
async def access_jwt_endpoint(access_token: str = Query(...), open_id: Optional[str] = Query(None)):
    """Convert an access token to JWT (requires open_id if not auto‑detected)."""
    ...
    # First, try to get open_id from the token if not provided
    if not open_id:
        try:
    ...
    # Try to get JWT using the same major_login function (needs region)
    # We'll try all regions
    for region in ALL_REGIONS:
        result = major_login(access_token, open_id, region)
        if result and result.get("jwt_token"):
