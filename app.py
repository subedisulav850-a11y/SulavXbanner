...
def wrap_text(text: str, font: ImageFont.FreeTypeFont, max_width: int) -> List[str]:
    """Split text into at most two lines at a natural break."""
    words = text.split()
    if len(words) <= 1:
        return [text]
    # Try to find a split point that fits both lines within max_width
    for i in range(1, len(words)):
        ...
        if w1 <= max_width and w2 <= max_width:
            return [line1, line2]
    # If no perfect split, return whole text
    return [text]
